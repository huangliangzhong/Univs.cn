...
public class DragGrid extends GridView {
	...
	/** 长按的时候ITEM的VIEW */
	private TextView dragItemView = null;
	/** WindowManager管理器 */
	...
	public void init(Context context) {
		...
		// 将布局文件中设置的间距dip转为px
		mHorizontalSpacing = ScreenSizeUtil.Dp2Px(context, mHorizontalSpacing);
	}
	...
	public void setOnItemClickListener(final MotionEvent ev) {
		setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				// return false;
				// }
				TextView dragTextView = (TextView) getChildAt(dragPosition
						- getFirstVisiblePosition());
				dragTextView.setSelected(true);
				dragTextView.setEnabled(false);
				itemHeight = dragTextView.getHeight();
				itemWidth = dragTextView.getWidth();
				...
				if (dragPosition != AdapterView.INVALID_POSITION) {
					...
					win_view_x = windowX - dragTextView.getLeft();// VIEW相对自己的X，半斤
					win_view_y = windowY - dragTextView.getTop();// VIEW相对自己的y，半斤
					...
					Bitmap dragBitmap = Bitmap.createBitmap(dragTextView
							.getDrawingCache());
					mVibrator.vibrate(50);// 设置震动时间
					startDrag(dragBitmap, (int) ev.getRawX(),
							(int) ev.getRawY());
					hideDropItem();
					dragTextView.setVisibility(View.INVISIBLE);
					isMoving = false;
	...
	public void OnMove(int x, int y) {
		...
		// 当 dPosition >1时是 判断下方的POSTION是否是最开始2个不能拖动的
		if (dPosition >= 0) {
			...
			if (dPosition != dragPosition) {
				// dragGroup设置为不可见
				View dragGroup = getChildAt(dragPosition);
				...
				for (int i = 0; i < movecount_abs; i++) {
					...
					View moveViewGroup = getChildAt(holdPosition);
						...
						public void onAnimationEnd(Animation animation) {
							...
							if (animation.toString().equalsIgnoreCase(
								...
								mDragAdapter.exchange(startPosition,
										dropPosition);
								if (exchangelis != null) {
									exchangelis.exchange(startPosition,
											dropPosition);
								}
	...
	private Exchangelistenter exchangelis = null;

	public void setonExchangelistenter(Exchangelistenter li) {
		this.exchangelis = li;
	}

	public interface Exchangelistenter {
		void exchange(int dragPostion, int dropPostion);
	}
}